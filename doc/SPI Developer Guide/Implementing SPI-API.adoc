...
The Method *authorisePsu* authorises psu and returns current (success or failure) authorisation status with *scaExempted* flag.It indicates whether the SCA should be exempted (possible for single and bulk payment initiation). *Should be used only with Embedded SCA Approach*. It contains following Data:
